...
public:
    ...
    T2 get(T1 k) const { return k2v.find(k)->second; }

    T1 get(T2 v) const { return v2k.find(v)->second; }
    ...
    T2 &operator[](T1 k) { return k2v[k]; }

    T1 &operator[](T2 v) { return v2k[v]; }
};
...
void timer_update(Timer_fd timer_fd,
                  std::chrono::nanoseconds start,
                  std::chrono::nanoseconds interval = 1000ns) noexcept
{
    const itimerspec its = {
        .it_interval = {.tv_sec = duration_cast<seconds>(interval).count(),
                        .tv_nsec = interval.count()},
        .it_value = {.tv_sec = duration_cast<seconds>(start).count(),
                     .tv_nsec = start.count()},
    };
...
Header &Pkt2header(Pkt &p)
{
    ...
    return *reinterpret_cast<Header *>(p.data.data());
}
...
Header &Pkt2header(Pkt_wrapper &p)
{
    ...
    return *reinterpret_cast<Header *>(p.buf.data());
}

Header &Pkt2header(std::vector<char> &v)
{
    if (v.size() < sizeof(Header)) [[unlikely]]
        throw std::length_error("Bad package size");

    return *reinterpret_cast<Header *>(v.data());
}
...
uint32_t pkt_mgr::recv(Pkt_wrapper &pp) noexcept
{
    ...
    return ::recvfrom(pp.fd, pp.buf.data(), pp.buf.size(), 0,
                      (sockaddr *) &pp.addr, &socklen);
}
...
void Mgr::regist(std::unique_ptr<Pkt_wrapper> &&p)
{
    if (p->buf.size() < sizeof(Header)) [[unlikely]]
        throw std::length_error(
            fmt::format("Bad package size {}", p->buf.size()));
...
uint32_t Mgr::send() noexcept
{
    ...
    for (int i = 0; i < nfds; i++) {
        ...
        tot_len += ::sendto(p->fd, p->buf.data(), p->buf.size(), 0,
                            (sockaddr *) &p->addr, sizeof(sockaddr_in));
    }
